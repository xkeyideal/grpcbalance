...
grpcbalance 的节点过滤场景通常会把端点元数据放进 `resolver.Address.Attributes`；你可以在“构造过滤器时解析 selector 一次”，在 Pick 时只做 `Matches()`。

如果你在本仓库中使用，优先直接使用 picker 层提供的 `LabelSelectorFilter(...)`（会把 attributes 适配为 `Labels`）。